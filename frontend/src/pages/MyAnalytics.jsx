        ...
        <div style={cardStyle}>
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ height: 8, background: 'var(--surface2)', borderRadius: 4 }}>
              <div style={{ height: '100%', width: `${rate}%`, background: rate >= 80 ? '#10B981' : rate >= 50 ? '#F59E0B' : '#EF4444', borderRadius: 4, transition: 'width 0.6s ease' }} />
            </div>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            }} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontFamily: 'Inter, sans-serif', color: 'var(--text)' }}>← Prev</button>
            <button onClick={() => {
              if (selectedMonth === 11) { setSelectedMonth(0); setSelectedYear(y => y + 1) }
              else setSelectedMonth(m => m + 1)
            }} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontFamily: 'Inter, sans-serif', color: 'var(--text)' }}>Next →</button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <div key={day} style={{
                minHeight: 70, padding: '6px', borderRadius: 8, border: '1px solid',
                borderColor: isToday ? '#4F46E5' : 'var(--border)',
                background: isToday ? '#EEF2FF' : 'var(--bg)'
              }}>
                <p style={{ fontSize: 12, fontWeight: isToday ? 700 : 500, color: isToday ? '#4F46E5' : 'var(--text)', marginBottom: 3 }}>{day}</p>
